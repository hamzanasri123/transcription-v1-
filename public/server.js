...
app.post('/transcrire', upload.single('video'), async (req, res) => {
    ...
    blobStream.on('finish', async () => {
        ...
        ffmpeg()
            ...
            .audioCodec('pcm_s16le') // le codec audio
            .audioFrequency(44100)   //  taux d'échantillonnage
            ...
            .on('end', async () => {
                ...
                blobWavStream.on('finish', async () => {
                    ...
                    // Transcrire l'audio avec l'API Google Cloud Speech-to-Text
                    ...
                    try {
                        ...
                        // Attendez que la reconnaissance soit terminée.
                        ...
                        fs.unlinkSync(audioOutput); // Suppression du fichier audio temporaire même en cas d'erreur
                ...
                // Pipe le fichier .wav dans le stream d'upload
